class Movie < ApplicationRecord
    ...
    scope :grossed_less_than, -> (mnt) { released.where("total_gross <= ? ", mnt) }
    scope :grossed_more_than, -> (mnt) { released.where("total_gross >= ? ", mnt) }
    ...
    private
      ...
      def acceptable_image
        return unless main_image.attached? && main_image.blob
        
        unless main_image.blob.byte_size <= 1.megabyte
          errors.add(:main_image, "is too big, please use size of 1 mb or lower")
        end
